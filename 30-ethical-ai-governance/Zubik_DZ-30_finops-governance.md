...
|---|---|---|
| **Легал-корпус права РБ** | `sft_train.jsonl` ≈ 400 МБ (сотни тысяч примеров) | правовые тексты и Q&A по законодательству Республики Беларусь (внешние правовые базы) |
...
|---|---|---|
| **Искусственный перевес идентичности (×10)** | модель склонна подчёркивать, что она «Зубр», и подтягивать тему платформы там, где её не спрашивали | 200 примеров дублированы десятикратно, чтобы не утонуть в сотнях тысяч легал-примеров — **сознательный трюк с известной ценой** |
| **Доменный сдвиг: право РБ ≠ ЛПА организации** | модель уверенно рассуждает о законодательстве, но внутренние регламенты знает только через ретрив | корпус — внешнее право, а не наши документы |
| **Однократная эпоха на большом корпусе** | адаптация неглубокая и неравномерная: частотные правовые сюжеты усвоены лучше редких | 1 эпоха, lr 1e-4 |
| **Vision не адаптирован** | распознавание сканов работает на «заводских» способностях базы; наши формы документов модель специально не училась читать | vision-башня заморожена |
| **Лицензионный характер корпуса** | обучающие данные — материалы внешних правовых баз с ограничениями на распространение | **следствие: веса адаптера являются производной от лицензионного контента** (см. риск в § 1.6) |
...
| **Запоминание лицензионного корпуса весами** — адаптер обучен на материалах внешних правовых баз с ограничениями на распространение; при публикации весов возможно извлечение дословных фрагментов | **Высокая** (юридическая, не приватности) | тест на memorization перед любой внешней публикацией; при подтверждении — публикация только GGUF-варианта либо ограничение доступа; во внутреннем контуре риск не реализуется |
