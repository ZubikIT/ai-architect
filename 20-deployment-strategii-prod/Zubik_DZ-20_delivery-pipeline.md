...
deadline: 2026-07-30
status: graded
grade: зачёт — «Домашняя работа принята» (И. Ящук, 16.07.2026)
---
...
**Илья Ящук · 16.07.2026, 04:11 — ✅ Работа принята**

> Добрый день, Александр!
>
> Домашняя работа принята. Работа получилась сильной и практичной: хорошо показано, как поставлять RAG-сервис не одной «монолитной» сущностью, а набором версионируемых артефактов — образ, веса модели, конфиг и версия векторного индекса.
>
> Отдельно отмечу связку с MLflow Model Registry, CI/CD-гейтами, RAGAS-проверками и canary-релизом. Видно, что вы продумали не только деплой, но и безопасный откат: отдельно для модели, индекса, конфига и сервиса.
>
> На будущее можно чуть подробнее раскрыть on-prem сценарий с малым числом GPU. Там, вероятно, практичнее сначала использовать shadow-режим, а canary включать уже осторожно и на коротком окне.
>
> Спасибо за выполненную работу!

**Замечание принято и по делу.** Четырёхчастный артефакт и независимый откат отмечены как основной вклад — то есть ставка сработала. Рекомендация про on-prem с малым числом GPU бьёт точно в наш случай: на 2× H100 canary означает, что часть и без того дефицитной ёмкости уходит под параллельную версию, а при выкатке второй реплики модели свободной карты уже не остаётся. Правильная последовательность для нас — **shadow (зеркалирование трафика без влияния на ответ) → короткое canary-окно → promote**, причём shadow можно гонять на батчевых окнах низкой нагрузки. Это стыкуется с выводом урока 21 о graceful degradation вместо HPA ([[ha-dr-genai]]) и с тезисом урока 24 о том, что скейлить инференс на двух картах некуда ([[highload-low-latency-inference]]). Учтено в плане доработки финального проекта.
