...
> **Как пользоваться файлом.** Версия на регламент **5 минут**: 11 слайдов. Строка «💬» — это дословная реплика с хронометражем, на слайд не выносится. Полная дека `presentation.md` (39 слайдов) остаётся резервом на вопросы. Сборка: `./scripts/md-to-slides.sh final-project/docs/presentation-5min.md`.
...
Retrieval: **вектор → обход 1–2 хопа → rerank**. Отмены попадают в контекст **безусловно**, вне конкуренции за места в top-k.

💬 **[1:44 → 2:18]** Отменяющий документ текстуально похож на десятки других — гарантии попадания в top-k у него нет. Сотрудник получает отменённую норму: без оговорки, с цитатой, то есть ошибку, которая выглядит обоснованной. Ответ — граф связей: четыре типа узлов, рёбра извлекаются правилами, детерминированно. Обход идёт после вектора и до реранка, поэтому отмена попадает в контекст безусловно — она может быть непохожа на вопрос, но именно она делает ответ верным.
...
**TCO** закрытого контура ≈ **$42,5k / 3 года**. Цена 1M токенов: **$0,89** при утилизации 100 % и **$17,78** при 5 %.

💬 **[3:44 → 4:16]** Вопрос «не тормозит ли ваш GraphRAG» закрыт цифрой: граф — три десятых процента. Время съедали генерация и реранк, и свой реранкер уехал в сервис платформы: на восьми клиентах в шесть раз больше RPS, но важнее, что узкое место сменило природу — из моих ядер в ожидание сети. В экономике решает утилизация: разброс себестоимости двадцатикратный, своё дешевле покупного примерно выше двадцати пяти процентов.
...
```
103 теста · 28 ADR · 9 диаграмм · 5 отчётов с цифрами · 3 518 строк кода
```

**Три отрицательных результата оставлены как есть:** мультиагентный слой не дал прироста · чужая цифра, взятая без проверки · рекомендация собственного отчёта оказалась не той.

**Чего нет:** граф и агентный слой **не встроены** в платформу (собраны, протестированы) · red teaming описан, не применялся · утилизация инференса не измерена.
