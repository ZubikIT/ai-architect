...
> **Как пользоваться файлом.** Слайд = блок между `---`. Строка «💬» — реплика докладчику, на слайд не выносится. Сборка: `./scripts/md-to-slides.sh final-project/docs/presentation.md`. Хронометраж: 53 слайда ≈ 30 минут. **Регламент защиты — 5 минут**, поэтому выступление идёт по `presentation-5min.md` (13 слайдов), а эта дека — резерв на вопросы.
...
## Guardrails: три слоя и один разрыв между ними

```mermaid
flowchart LR
  q(["Запрос"]) --> g1["<b>Маскирование ПДн</b><br/>прокси перед шлюзом<br/><i>спроектирован, не встроен</i>"]
  g1 --> gw["Шлюз моделей<br/><b>граница контура</b> — работает"]
  gw --> g2["Встроенные guardrails LiteLLM<br/><i>доступны, не включены</i>"]
  g2 --> m["Модель"]
  subgraph own["Курсовой контур — работает"]
    i["Guardrail входа: инъекции"]
    o["Guardrail выхода: маска ПДн на потоке"]
  end
  classDef planned stroke-dasharray: 6 4
  class g1,g2 planned
```
...
| Слой | Где | Статус |
|---|---|---|
| Границa контура — модель разрешена ключу | шлюз | ✅ **работает**, проверяется до вызова вендора |
| Вход: инъекции · выход: маска ПДн на потоке | курсовой контур | ✅ работает, покрыто тестами |
| Маскирование ПДн с рехидратацией, fail-closed, SIEM | прокси перед шлюзом | ⬜ **спроектирован, не развёрнут** |
| Встроенные guardrails LiteLLM | шлюз | ⬜ не настроены |

**Путь запроса проверен целиком:** `api.zubriq.by` → обратный прокси → ingress → **стоковый образ LiteLLM, один контейнер в поде**. Между клиентом и моделью нет ни одного нашего компонента — sidecar'у перехватить нечего.

💬 Спроектированный слой описан детально: замена УНП, карт, счетов и ФИО на синтетику до отправки в модель и рехидратация в ответе, устойчивые пары внутри диалога, контрольные суммы УНП по модулю одиннадцать, fail-closed при отказе модуля, события в SIEM. Это хорошее решение, и оно не моё — это решение платформы. Но статус я обязан назвать, и он проверен не грепом по конфигу, а составом пода: в нём один контейнер, стоковый образ шлюза, и оба публичных имени ведут в него напрямую через ingress. Перехватывать запрос по дороге нечем. Работает из всей конструкции ровно один барьер: проверка, разрешена ли модель ключу. Она и держит границу контура — и её достаточно для критерия ТЗ про облачные API, но не достаточно для обещания про персональные данные.
...
## Своя дообученная модель: Zubr-VL-32B

QLoRA поверх `Qwen3-VL-32B-Instruct` на праве Республики Беларусь. **Опубликована** — `ArtCloud/Zubr1.5-VL-32B`.

| | |
|---|---|
| Корпус | **757 НПА** (28 кодексов) + 5 545 аналитических статей, 133,4 млн символов |
| Обучение | r=16, α=32, 896 тензоров = **0,4 %** параметров базы; 1 эпоха, 1× H100 |
| Артефакты | адаптер 269 МБ · GGUF Q4_K_M 19,8 ГБ · Model Card |
| Сервинг измерен | 31–33 ток/с на 4× V100, ~27 ГиБ при контексте 16k |
| **Проверка на memorization** | корпус в шинглах по 80 символов: **97,6 % фрагментов встречаются ровно один раз** |

> **Оговорка, без которой это было бы рекламой: vision-башня при обучении заморожена, корпус текстовый.** Значит *видит* страницу она ровно как базовая Qwen3-VL. Выигрыш — в языке домена: терминология, структура НПА, формулы ссылок и отмен. Поэтому в конвейере ([ADR-0014](adr/0014-multimodalnyy-ingestion.md)) она стоит на извлечении связей, а не на распознавании.

**Честный статус:** в каталоге шлюза модель **не заведена** — алиас `zubr` указывает на зеркало. Прямой замер memorization на ней самой не сделан: юнит объявляет `Conflicts=` с боевым, все четыре карты заняты.

💬 Это единственная часть работы, где обучалась модель, а не собиралась система, и её легко было бы подать как главное достижение. Но замер запрещает: заморожённая vision-башня означает, что зрение у неё базовое, и в мультимодальном конвейере она полезна не тем, ради чего её обычно берут. Тест на memorization я делал не на самой модели, а на корпусе — проверял, есть ли чему заучиваться: девяносто семь и шесть десятых процента фрагментов встречаются ровно один раз. Это нижняя граница риска, а не доказательство его отсутствия, и в Model Card записано именно так.
...
## Две модели на одном стенде

18.09 модель контура сменилась: MoE `Qwen3.6-35B-A3B` → плотная `Qwen3.8-27B`. Перемерил на живом стеке тем же инструментом, 25.09.

| Шаг `ask` | Qwen3.6 · 12.09 | **Qwen3.8 · 25.09** | |
|---|---:|---:|---:|
| **Генерация** | 1 611 мс | **1 933 мс** | **+20 %** |
| Реранк (CPU стенда) | 708 мс | 1 423 мс | ×2,0 |
| Обход графа | 5,5 мс | 12 мс | 0,35 % ответа |
| **Итого** | 2 368 мс | 3 468 мс | +46 % |

| Движок, 24 клиента | Qwen3.6 | **Qwen3.8** |
|---|---:|---:|
| Выходных ток/с | 200 | **218** (+9 %) |
| Первый токен, медиана | **0,59 с** | 1,31 с |

> **Быстрее на бенчмарке, медленнее в продукте.** Плотная модель печатает быстрее, но дольше до первого токена, а у RAG длинный промпт и короткий ответ.

💬 Две трети прироста дал стенд, а не модель: реранк и эмбеддер считаются на CPU общего devbox и стали вдвое медленнее при тех же моделях. Поэтому модели я сравниваю по спану генерации, а не по итогу. Чистого A/B нет: для него надо дважды перезапустить прод-vLLM, по пять минут без локальной модели. Строгий SLO в три секунды не держится и за вычетом стенда — это уже цена модели.
...
💬 Метрики движка показывают, как работает модель, но не кто её зовёт; это видно только на шлюзе, по ключу. Шлюз и движок сходятся по токенам до трёх процентов. Главный вывод неожиданный: основную нагрузку создаёт не человек, а фоновый сканер, и хвост первого токена у чата, скорее всего, его очередь. Реранк на сервис платформы я перенёс и перемерил: сегодня сервис на CPU, и стало в два с половиной раза медленнее, поэтому откатил.
...
## Эмбеддер на GPU: перенос вскрыл несуществующий порог

Локальная MiniLM-384 выбиралась под CPU, а не по качеству. Сервис платформы отдаёт **`bge-m3`** — тот самый, которым считались все замеры качества на боевом корпусе.
...
**У MiniLM зазор отрицательный.** Чужой вопрос ближе к корпусу, чем худший свой: порога, разделяющего одно от другого, на этой модели **не существует** — любое значение либо режет свои вопросы, либо пропускает чужие.

Латентность: медиана 108 → **59 мс**, p90 **159 → 61 мс** — исчез хвост, а не медиана.

💬 Порог ноль сорок пять был записан в ADR по замеру «ноль пятьсот восемьдесят пять против ноль триста шестьдесят девять» — и он был верен, пока в эталоне не появился вопрос про журнал аудита: текстуально близкий к корпусу, но ответа на него там нет. На лёгких отрицательных примерах вроде «что приготовить из курицы» некалиброванность не видна — ровно тот же класс дефекта, что я нашёл у реранкера ms-marco, и я на него наступил второй раз, уже зная про первый. Зазор у bge-m3 тонкий, двадцать шесть тысячных, поэтому роли не меняются: косинус остаётся дешёвым ранним выходом, решает калиброванный реранкер.
...
xychart-beta
    ...
    y-axis "Цена, $" 0 --> 30
    bar [28.2, 14.1, 5.64, 2.82, 1.41]
    line [2, 2, 2, 2, 2]
```

Линия — **потолок коридора внешнего вендора** ($0,3–2 за 1M выходных). Порог окупаемости — **~70 % утилизации**: $9,8k в год ÷ $2 за 1M = 4 900 млн токенов при потолке ~220 ток/с (Qwen3.8, 24 запроса одновременно). До смены модели, при 506 ток/с и 96 запросах, — ~31 %.
...
| Месячный объём на полке 506 ток/с (Qwen3.6) · ~220 ток/с (Qwen3.8) | **44 минуты** · **~1 ч 42 мин** |
...
- ~~эмбеддер на CPU~~ → **перенесён на сервис платформы**, и это вскрыло, что порога отказа на старой модели не существовало
...
- **слой маскирования ПДн перед шлюзом** — спроектирован платформой, не развёрнут; встроенные guardrails шлюза пусты
