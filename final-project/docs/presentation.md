...
Порог отказа поставлен на косинус эмбеддера: логиты реранкера не разделяют вовсе.

💬 Все пять видно только при реальном прогоне. На схемах ни одна не заметна.
...
- **реранк остаётся узким местом** — модель английская, на ONNX не переведена
...
4. **Реранк на ONNX и в отдельный пул** — 30–76 % латентности в одном компоненте
