...
> ⚠️ **Расходится с продом — по поколению, уже не по архитектуре.** В работе `Qwen3.8-27B-AWQ-INT4` (сверено 18.09.2026): плотная модель на 27 млрд, то есть **тот самый класс, который выбирал этот ADR**. До августа 2026 прод шёл на MoE `Qwen3.6-35B-A3B-AWQ` — следствие железа ([ADR-0006](0006-kvantovanie-i-sizing-gpu.md)). Осторожно: шлюз держит `qwen3.6-35b` алиасом на веса 3.8, по имени модели реальные веса не определяются ([сверка](../adr-audit.md)).
