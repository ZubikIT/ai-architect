...
- **Железо:** consumer / enterprise GPU без кластера H100 → обязательны **квантование (AWQ / GGUF)** и **оптимизация KV-cache** (требование брифа).
...
- **Air-gapped / 152-ФЗ:** vLLM работает офлайн с предзагруженными весами, без обязательной телеметрии — соответствует контуру ADR-0001. На сетевом уровне исходящий трафик движка блокируется.
