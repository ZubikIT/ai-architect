...
## Кому это нужно: два класса вопросов об отменах

Уточнение от 13.09.2026, появившееся из спора с платформой и едва не стоившее этому решению обоснования.

Платформа измерила спрос на исторические вопросы к праву по своему продуктивному трафику: из **3 738 сообщений пользователей — ноль** вопросов вида «что действовало до поправки», и в её эталоне из 61 вопроса таких тоже нет. Вывод напрашивался: механизм отмен настоящий, но спроса на него не показано.

**Вывод неверен, потому что смешивает два разных класса вопросов.**

| | Класс A | Класс B |
|---|---|---|
| Пример | «Что действовало до поправки?» | «Сколько дней отпуска?» |
| О чём спрашивают | о прошлом, явно | о сегодняшнем дне |
| Замеренный спрос | **0 из 3 738** | каждый вопрос к изменённой норме |
| Что даёт отсутствие графа | нет ответа | **уверенно неверный ответ** |

Ребро `ОТМЕНЯЕТ` в этой системе обслуживает **класс B**, а не класс A. Проверено на демо-корпусе, где ЛПА-04 § 1 меняет «28 календарных дней» на 30:

```
вопрос: Сколько дней основной ежегодный трудовой отпуск?

граф ВКЛЮЧЁН    источник ЛПА-01 § 1 · пометка: отменён документом ЛПА-04
граф ВЫКЛЮЧЕН   источник ЛПА-01 § 1 · пометок нет
```

Ранжирование в обоих случаях ставит первым **отменённый** пункт — он ближе к вопросу по словам, и никакая нарезка этого не меняет ([ab-anchor](../eval/ab-anchor.md)). Без графа пользователь получает «28 календарных дней» без единой оговорки, и ответ выглядит ровно так же уверенно, как правильный.

**Спрос на класс B нельзя измерить по логам** — пользователь не формулирует его как вопрос об отмене, он просто спрашивает про норму. Его «спрос» равен доле вопросов, попадающих в изменённые нормы, и для корпуса ЛПА с приказами о внесении изменений это не редкий случай, а обычный.

Отсюда точная формулировка пользы графа, которую и следует защищать: **не «система умеет отвечать на исторические вопросы», а «система не выдаёт отменённую норму за действующую»**. Первое — функция, на которую спроса не показано. Второе — свойство корректности, которое требуется всегда.

### И сразу оговорка, без которой формулировка становится обещанием

**Гарантия распространяется только на отмены, известные корпусу.**

Граф строится по тому, что загружено, и ребро `ОТМЕНЯЕТ` выводится правилами из текста загруженных документов. Если норма отменена актом, которого в корпусе **нет**, она останется действующей, и ни один механизм этого не заметит — сравнивать не с чем.

Это не теоретическая придирка. Проверка на боевом корпусе платформы (8 635 актов: законы, указы, постановления) показала расклад, которого мы оба не ожидали: пар «акт + акт, вносящий изменения» там практически нет — документов «о внесении изменений» **6 из 8 635**, тексты лежат консолидированными редакциями. То есть класс B у них сегодня не срабатывает вовсе: отменённой нормы в базе просто нет.

Зато там доминирует другой риск той же формы: корпус — это **8 635 снимков, взятых в момент загрузки**. Акт, отменённый после загрузки чем-то, чего не выгружали, остаётся в базе действующим. Тот же «уверенно неверный ответ прямо сейчас», только источник расхождения — внешний мир, а не вторая редакция внутри корпуса. **Граф здесь бессилен по построению: он видит только загруженное.**

### Чего у нас нет и почему это не заглушено

У `Document` нет ни даты редакции, ни отметки о времени загрузки, ни срока актуальности — вообще никакого понятия свежести. Даты существуют только внутри заголовка текстом («приказ № 117 от 12.03.2026») и ни на что не влияют.

Поставить сюда отметку времени загрузки было бы **декоративной работой**: она ответит на вопрос «когда мы это прочитали», а нужный вопрос — «не изменилось ли это с тех пор», и на него отметка не отвечает. Честный механизм требует того, чего в этом проекте нет: источника, который сообщает об изменениях (подписка на ленту правовых актов), и сверки с ним.

Поэтому вместо заглушки — прямая запись в ограничениях: **система гарантирует, что не выдаст норму, отменённую внутри корпуса; она не гарантирует, что корпус актуален.** Второе — задача ingestion, и её не решить на стороне retrieval.

### Наличие механизма свежести не есть гарантия свежести

Оговорка к оговорке, и она дороже обеих. На платформе механизм сверки **существует и устроен правильно**: команда обновления перекачивает исходный адрес, считает sha1 текста и сравнивает с сохранённым хешем — то есть поймала бы и отмену, вшитую в консолидированную редакцию, потому что сверяется содержимое, а не наличие документа.

Замер охвата на живых данных:

| | |
|---|---:|
| документов в базе | 12 252 |
| из них с адресом источника | 8 666 |
| **без источника — не обновятся никогда** | **3 586 (29 %)** |
| сверялись с источником хоть раз | 1 228 (10 %) |
| темп сверки | 300 в неделю |
| первый полный круг | ещё **25 недель** |

Механизм правильный. Практическая гарантия близка к нулю: девять документов из десяти ни разу не сверялись, почти треть не сверится никогда — им неоткуда.

**По коду этого не видно. Видно только по замеру.** И это опаснее отсутствия механизма: наличие правильно написанной сверки закрывает вопрос в голове у того, кто её читал, и он не спросит, сколько документов ею покрыто.

Отсюда критерий приёмки для этого пункта дорожной карты — **не «сделана подписка», а две цифры**: доля документов с адресом источника и возраст самой давней сверки. Первая говорит, какая часть корпуса в принципе обновляема; вторая — насколько устарело то, что обновляемо.
