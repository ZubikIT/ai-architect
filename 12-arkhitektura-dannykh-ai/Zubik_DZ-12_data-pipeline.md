...
### Критерии приёмки (из ЛК)
Статус «Принято», если:
- [x] **Выбор технологий:** инструменты соответствуют задачам (Stream vs Batch) — §1, §3.
- [x] **Полнота потока:** данные прослеживаются от источника до модели — схема §2.
- [x] **Feature Store:** понимание роли для предотвращения Training-Serving Skew — §3, §4.

### Сборка PDF
Путь B из [[md-to-pdf-toolchain]]: mermaid.ink → PNG (`curl` с браузерным UA, `?type=png&width=1400&scale=2`) → подмена блока на картинку → `scripts/md-to-pdf.sh` (pandoc + typst). Диаграмма — TB: широкая LR на A4 нечитаема.

### Обратная связь от преподавателя
_(после проверки; сдано 11.06.2026)_
