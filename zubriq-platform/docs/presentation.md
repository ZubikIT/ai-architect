...
- Модель: **Qwen3.8-27B, плотная, INT4** (с 18.09.2026 вместо MoE 35B-A3B), tensor-parallel по 4 картам, окно 262K
- Одна модель на текст **и** зрение — вечное имя `zubr`, смена весов не трогает клиентов
- Веса — в объектном хранилище, синхронизация на бокс при старте
- Предыдущая модель оставлена как путь отката

> **VRAM эксклюзивна.** Второй жилец и прод на одном боксе не сосуществуют: пилот свёрнут, VL-модель погашена, эмбеддинги ушли на CPU.

💬 MoE выбирали, считая, что плотная модель на V100 не потянет. Замер при свопе опроверг: плотная 27B на 9 % быстрее на генерации, платим только первым токеном — 1,3 с против 0,6.
