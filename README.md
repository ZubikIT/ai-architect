...
| [`zubriq-platform/`](zubriq-platform) | Архитектура работающей платформы ZubrIQ (`zubriq.by`) — описание as-is: шлюз моделей, два режима доступа, переезд в кластер |
| [`kurs/`](kurs/README.md) | **Сквозной разбор курса**: [полный](kurs/kurs-polnyy.md) (идея урока → где применено → что показал замер, шесть расхождений курса с замером), [выжимка на страницу](kurs/kurs-kratko.md) а также [программа своего курса](kurs/programma.md) (44 занятия) и [интенсив](kurs/lms-syllabus.md) (12 модулей) для `lms.artcloud.by` |
| [`landing/`](landing/index.html) | **Лендинг защиты** — одна страница с проигрывателем демо, диаграммами и отчётами; публикуется на Pages как `/landing/` |
