...
(status: accepted) — это не учебный пример, а реальное решение для **Суфлёра** (публичная копия решения для ЗАО МТБанк, zubriq.by). Ниже — версия ADR под формулировку ДЗ (SaaS GPT-5 vs self-hosted Llama 3), дополненная **сравнением по 5 критериям** и **питчем для CTO**, которых требует задание.
...
### Decision
Используем **self-hosted open-weight LLM** (класс Llama 3 / Qwen / DeepSeek / T-lite) на движке инференса (vLLM/SGLang) внутри изолированного контура. Внешние SaaS-API (GPT-5 и аналоги) **запрещены**.

**Сравнение вариантов по критериям задания:**

| Критерий | SaaS (GPT-5 / проприетарный API) | **Self-Hosted (Llama 3, on-prem)** ✅ |
|---|---|---|
