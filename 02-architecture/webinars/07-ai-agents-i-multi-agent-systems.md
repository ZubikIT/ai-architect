...
>
> Артефакты: [слайды (PDF)](../artifacts/lesson-07-ai-agents.pdf) · [демка `7-agents`](../artifacts/lesson-07-agents-demo/README.md) (иерархический MAS на LangGraph: main supervisor → research team из 4 ReAct-агентов через safron.io API + editing team `fact_checker → summarizer`, общий notes-файл; YandexGPT через OpenAI-совместимый API) · [ДЗ-07](../homework/07-ai-agents.md) + [Colab `07-ai-agents.ipynb`](../homework/07-ai-agents.ipynb) (TripBuddy: MAS + RAG для оформления командировок, mock-режим запускается «cold»).
>
