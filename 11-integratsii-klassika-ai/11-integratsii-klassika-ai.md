...
- **Latency:** REST ждёт ~200 мс, LLM — секунды; **HTTP-таймауты убивают соединения**.
...
- **Тайм-ауты:** дефолт HTTP-клиента ~30 с — для LLM мало. Ставим **агрессивный на connect (~2 с)** и **мягкий на read (60+ с)**.
...
**Правило: никогда не слать PII (ФИО, полные номера карт) в публичные LLM (OpenAI/Anthropic).** Варианты: **псевдонимизация** чувствительных данных перед отправкой в облако; **либо локальные модели on-premise**, где данные не покидают контур. ← прямое обоснование нашего on-prem-выбора (РБ, [[final-project-hardware]]).
...
- **«Универсальный драйвер»:** **Server** оборачивает реальный источник (Postgres, Slack, FileSystem) в стандартные объекты MCP (**Resources, Tools, Prompts**); **Client (LLM)** не знает специфики Postgres, но умеет звать `tools/call`.
