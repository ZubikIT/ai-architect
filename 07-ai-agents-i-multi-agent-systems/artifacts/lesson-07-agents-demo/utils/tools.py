...
def fetch_keywords_data(period="daily", category=None, limit=3, sort="trending"):
    ...
    try:
        response = requests.get(
            "https://public.api.safron.io/v2/keywords", params=params, timeout=60
        )
...
def get_or_create_notes_file():
    ...
    if CURRENT_NOTES_FILE is None:
        ...
        CURRENT_NOTES_FILE = NOTES_DIR / filename

        with open(CURRENT_NOTES_FILE, "w", encoding="utf-8") as f:
            f.write(
                f"# Research Notes - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
            )

        print(f"\n----- CREATED NEW NOTES FILE: {CURRENT_NOTES_FILE} -----\n")

    return str(CURRENT_NOTES_FILE)
